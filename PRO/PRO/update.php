...
<div class="topnav">
  ...
  <a class="active" href="\project2019-2020\PRO\PRO\update.php">Delete/Show/Update Questions</a>
...
<?php
  ...
  if($st==1)
  {
  $query = "SELECT  * FROM `$CID` WHERE USERNAME = '$userlogin' AND
    BIT LIKE '1%'ORDER BY BIT ASC"; 
    //echo "HIII";
  } 
  elseif ($st==2) {
     $query = "SELECT  * FROM `$CID`  WHERE  USERNAME = '$userlogin' AND
    BIT LIKE '2%'ORDER BY BIT ASC"; 
  }
  elseif ($st==3) {
     $query = "SELECT  * FROM `$CID`  WHERE  USERNAME = '$userlogin' AND
  ...
  {
    $query = "SELECT  * FROM `$CID`  WHERE  USERNAME = '$userlogin' ORDER BY 'BIT' ASC"; 
  }
